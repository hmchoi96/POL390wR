...
#From the codebook: 998. Don’t know (don’t know where to rate), -9. Refused and -4. Technical error         ##if number is < -1, turn them to NA, if not, stay
if_else(minidat$FeelingBiden < 0 | minidat$FeelingBiden > 100, true= NA_real_, false = minidat$FeelingBiden)
if_else(minidat$FeelingBiden<0 | minidat$FeelingBiden>100, NA_real_, minidat$FeelingBiden) %>% summary()

minidat$FeelingBiden <- if_else(minidat$FeelingBiden < 0 | minidat$FeelingBiden > 100, true= NA_real_, false = minidat$FeelingBiden)
#this looks better. 
...
minidat$FeelingTrump %>% summary() #Don't know where to rate is gone, we only have people who refuse to say! 
...
TrumpRate <- TrumpRate+theme(legend.position = "none") # 
cowplot::plot_grid(TrumpRate, BidenRate) #now we only have one, although it is still redundant because of our x-axis labels. 
```
...
minidat %>%  
  mutate(
    Votes = case_when(
      #I'm going to categorize the combination fo 2016 and 2020 votes. 
      ...
      TRUE ~ NA_character_), #the last condition always has to be TRUE ~ and an answer. 
  ...
  group_by(Votes) %>%
  group_modify(~ mean_se(.x$`Thermometer difference`))   #.x is used to tell it to use the result of the previous function (group_by(Votes))
...
#Pick one and try to output the plot:
  ...
  ggplot(aes(x = Votes, y = y, fill = Votes, 
             color = Votes)) +
  geom_col(position = position_dodge(0.9)) +
  geom_errorbar(aes(ymin = ymin, ymax = ymax ), width = 0.5, position = position_dodge(0.9), color = "black") +
  scale_y_continuous("Mean rating difference")+
  ggtitle("Vote change and rating of Trump in 2020")
